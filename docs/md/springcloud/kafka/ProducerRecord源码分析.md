...
该类的实例用于存放生产者要向kafka集群发布的消息记录。

- topic：主题名称，用来给消息记录做逻辑层面的一级划分，即该消息记录应该被发布到kafka集群的哪个类别（主题）下
- partition：分区号，用来给消息记录做逻辑上二级划分，即该消息记录应该被发布到kafka集群指定类别下的哪个队列（分区）中
...
public int partition(String topic, Object key, byte[] keyBytes, Object value, byte[] valueBytes, Cluster cluster,
        ...
        // keyBytes 不为空，计算key的hash并取模(求两个数相除的余数)获得分区
